...
def yahoo_finance_to_gcs(ds, **kwargs):
    ...
    for symbol in SYMBOLS:
        try:
            # Criar objeto ticker do Yahoo Finance
            ticker = yf.Ticker(symbol)
            
            # Buscar dados históricos do dia anterior
            data = ticker.history(
                start=previous_day, 
            ...
            if not data.empty:
                ...
                stock_data = {
                    ...
                    "timestamp": int(previous_day.timestamp() * 1000)  # Para compatibilidade
                }
...
with DAG(
    ...
    # Aguardar que os arquivos sejam carregados em raw/ERP no bucket GCS
    ...
    # Preparar Dataproc serverless e executar o job de transformação
    ...
    # Adicionar dados de preços de ações ao BigQuery com append
